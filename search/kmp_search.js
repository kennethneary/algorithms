...
function buildPrefixTable(s) {
  ...
  while (i < s.length) {
    ...
    if (s[i] === s[j]) {
      ...
      // If the characters do no match, and we have repetition in suffix and prefix,
      // we still need to check the character after the prefix
      // look up table will give you the prefix index
      j = table[j - 1];
    } else {
...
function kmpMatching(string, substring) {
